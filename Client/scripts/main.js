...
function displayContests() {
    ...
    if (shouldBeDisplayed > indContestDivs.length) {
        shouldBeDisplayed = indContestDivs.length;
    }
    /* Append more contests into contestsDiv and increment numContestsDisplayed each time. */
    for (; numContestsDisplayed < shouldBeDisplayed; numContestsDisplayed ++) {
        contestsDiv.appendChild(indContestDivs[numContestsDisplayed]);
    }
    /* Hide moreContestsBtn when there are no more contests. */
    if (numContestsDisplayed === indContestDivs.length) {
        moreContestsBtn.style.display = "none";
    }
    /* Otherwise, show it. */
    else {
        moreContestsBtn.style.display = "block";
    }
}
